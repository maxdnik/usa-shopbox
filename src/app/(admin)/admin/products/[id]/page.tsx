...
import { CATEGORIES_MAP } from "@/lib/constants/categories";

const MAIN_OPTIONS = CATEGORIES_MAP.map((c) => c.name);

function getSubOptions(main: string) {
  return CATEGORIES_MAP.find((c) => c.name === main)?.subcategories?.map((s) => s.name) ?? [];
}

function getLeafOptions(main: string, sub: string) {
  return (
    CATEGORIES_MAP.find((c) => c.name === main)
...
export default function EditProductPage() {
  ...
  const addVariation = () => {
    setProduct({
      ...product,
      variations: [...(product.variations || []), { attribute: "Color", value: "", stock: 1 }],
    });
  };
  ...
  const removeVariation = (index: number) => {
    setProduct({
      ...product,
      variations: product.variations.filter((_: any, i: number) => i !== index),
    });
  };
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-[#0A2647] flex items-center justify-center text-white font-black tracking-widest uppercase">
        Abriendo Editor Maestro...
      </div>
    );
        ...
        <div className="flex justify-between items-end mb-10">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <p className="text-[#0A2647] text-[10px] font-black uppercase tracking-widest opacity-40">
                Editor Maestro de Inventario
              </p>
            </div>
          ...
          <button
            type="submit"
            disabled={saving}
            className="bg-[#0A2647] text-white px-12 py-5 rounded-[24px] font-black shadow-2xl hover:scale-105 transition disabled:opacity-50"
          >
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white p-10 rounded-[48px] border shadow-sm space-y-8">
              <h3 className="text-xs font-black text-slate-300 uppercase tracking-widest border-b pb-4">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  >
                    ...
                    {getSubOptions(product.category.main).map((sub) => (
                ...
                <div>
                  ...
                  >
                    ...
                    {getLeafOptions(product.category.main, product.category.sub).map((leaf) => (
              ...
              <div className="grid md:grid-cols-2 gap-8 pt-4">
                <div>
                  ...
                  <input
                    className="w-full border-b py-2 text-sm font-bold text-slate-600 outline-none"
                    value={product.store}
                ...
                <div>
                  ...
                  <input
                    className="w-full border-b-2 py-2 text-xl font-bold text-slate-800 outline-none focus:border-[#0A2647]"
                    value={product.title}
              ...
              <div>
                ...
                <textarea
                  rows={5}
                  className="w-full border border-slate-100 rounded-[24px] p-6 mt-4 text-sm text-slate-600 outline-none focus:border-[#0A2647] shadow-inner"
            ...
            <section className="bg-white p-10 rounded-[48px] border shadow-sm">
              <h3 className="text-xs font-black text-slate-300 uppercase tracking-widest border-b pb-4 mb-8">
                Galería de Imágenes (Local / eBay)
              </h3>
              ...
              <div className="grid grid-cols-1 gap-4 mb-8">
                {product.images?.map((img: string, idx: number) => (
                  <div
                    key={idx}
                    className="flex items-center gap-6 bg-slate-50/50 p-4 rounded-[24px] border border-slate-100"
                  >
                    ...
                    <button
                      ...
                      className="text-red-300 hover:text-red-500 transition px-2"
                    >
                      <svg
              ...
              <div className="flex gap-4 items-end bg-[#0A2647]/5 p-8 rounded-[32px] border border-[#0A2647]/10">
                <div className="flex-1">
                  <label className="text-[10px] font-black text-[#0A2647] uppercase tracking-widest mb-2 block">
                    Vincular Nueva Foto
                  </label>
                  <input
                    type="text"
            ...
            <section className="bg-white p-10 rounded-[48px] border shadow-sm">
              <div className="flex justify-between items-center mb-8 border-b pb-4">
                ...
                <button
                  ...
                  className="bg-blue-50 text-blue-600 px-5 py-2 rounded-full text-[10px] font-black hover:bg-blue-100 transition tracking-widest"
                >
                  + AGREGAR FILA
                </button>
              </div>
              <div className="space-y-4">
                {product.variations?.map((v: any, idx: number) => (
                  <div
                    key={idx}
                    className="flex gap-6 items-end bg-slate-50/50 p-6 rounded-[24px] border border-slate-100"
                  >
                    <div className="w-1/3">
                      <label className="text-[9px] font-bold text-slate-400 uppercase">Atributo</label>
                      <select
                        className="w-full bg-transparent border-b py-2 text-xs font-black text-[#0A2647]"
                        value={v.attribute}
                        onChange={(e) => {
                    ...
                    <div className="flex-1">
                      ...
                      <input
                        className="w-full bg-transparent border-b py-2 text-xs font-bold"
                        value={v.value}
                        onChange={(e) => {
                          const updated = [...product.variations];
                    ...
                    <button
                      ...
                      className="text-red-300 hover:text-red-500 transition mb-2"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
          ...
          <div className="space-y-8">
            ...
            <section className="bg-emerald-600 p-10 rounded-[48px] text-white shadow-2xl shadow-emerald-200/50">
              <h3 className="text-[10px] font-black uppercase tracking-widest opacity-70 mb-6">
                Costeo de Importación
              </h3>

              <div className="grid grid-cols-2 gap-4 mb-8">
                <div>
                  <label className="text-[9px] font-black uppercase opacity-60 block mb-2">
                    Precio Tienda USA
                  </label>
              ...
              <div className="space-y-6">
                {fields.map((f: string) => (
                  <div key={f}>
                    ...
                    <input
                      className="w-full border-b py-2 text-xs font-bold text-slate-700 outline-none focus:border-emerald-500 transition"
                      value={product.specs[f] || ""}
                      onChange={(e) =>
                        setProduct({ ...product, specs: { ...product.specs, [f]: e.target.value } })
                      }
                    />
                  </div>
                ))}
