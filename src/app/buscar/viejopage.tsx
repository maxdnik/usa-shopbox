...
export default function SearchPage() {
  ...
  const dynamicFilters = useMemo(() => {
    ...
    const stores = Object.entries(storeCounts).map(([name, count]) => ({ name, count }));
    ...
    const defaultPath = { main: "Tecnología", sub: "Celulares y Teléfonos", leaf: "Celulares y Smartphones" };
  ...
  const filteredItems = useMemo(() => {
    let result = items.filter(item => {
      const matchesStore = !selectedStore || (item.brand === selectedStore || item.store === selectedStore);
      const itemPrice = item.isInternal ? item.priceUSD : (typeof item.price === "number" ? item.price : Number(item.price));
        ...
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row gap-12">
          ...
          {(!loading && filteredItems.length > 0) && (
            <aside className="w-full md:w-64 flex-shrink-0">
              <h1 className="text-3xl font-black text-[#0A2647] mb-2 capitalize tracking-tighter leading-none">{query}</h1>
              <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-6">{filteredItems.length} productos encontrados</p>
              ...
              <div className="space-y-6">
                ...
                <div className="pt-2 border-t border-slate-100">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h3 className="text-[14px] font-black text-[#1E3A8A] uppercase tracking-widest">Filtrar por tienda</h3>
                  </div>
                  <div className="flex flex-col gap-2 pl-4">
                    {dynamicFilters.stores.map(store => (
                      <button 
                        ...
                        className={`text-[13px] text-left hover:text-red-600 transition-colors uppercase font-black tracking-tight ${selectedStore === store.name ? 'text-red-600' : 'text-slate-400'}`}
                      >
                        {store.name} <span className="text-[10px] opacity-40 ml-1">({store.count})</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                {dynamicFilters.attributes.map(attr => (
                    <div key={attr.name} className="border-t border-slate-100 pt-2">
                        <div className="flex items-center gap-3 mb-3">
                            <div className="w-[4px] h-5 bg-red-600 rounded-full"></div>
                            <h3 className="text-[14px] font-black text-[#1E3A8A] uppercase tracking-widest">{attr.name}</h3>
                        </div>
                        <div className="flex flex-col gap-2 pl-4">
                            {attr.options.map(opt => (
                                <button
                                    ...
                                    className={`text-[13px] text-left font-black uppercase tracking-tight ${selectedSpecs[attr.name] === opt.val ? 'text-red-600' : 'text-[#0A2647] hover:text-red-600'}`}
                                >
                                    {opt.val} <span className="text-[10px] opacity-40 ml-1">({opt.count})</span>
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <div className="border-t border-slate-100 pt-2">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-[4px] h-5 bg-red-600 rounded-full"></div>
                    <h3 className="text-[14px] font-black text-[#1E3A8A] uppercase tracking-widest">Precio máximo</h3>
                  </div>
                  <div className="flex flex-col gap-2 pl-4">
                     {[500, 1000, 2000].map(p => (
                       <button 
                        ...
                        className={`text-[13px] text-left font-black uppercase tracking-tight ${maxPrice === p ? 'text-red-600' : 'text-[#0A2647] hover:text-red-600'}`}
                       >
                        Hasta USD {p}
                       </button>
                     ))}
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center min-h-[60vh]">
                <div className="bg-white p-12 rounded-[48px] shadow-2xl border border-slate-100 flex flex-col items-center gap-8 max-w-lg w-full">
                  ...
                  <div className="text-center space-y-3">
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">Buscando "{query}"</p>
                    <h2 className="text-2xl font-black text-[#0A2647] uppercase tracking-tighter leading-none">Estamos buscando las <br /> mejores publicaciones</h2>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-10">
                <div className="bg-white rounded-[48px] p-16 border border-slate-100 shadow-sm flex flex-col items-center text-center">
                  ...
                  <h2 className="text-3xl font-black text-[#0A2647] uppercase tracking-tighter mb-4">
                    No encontramos resultados para <br /> <span className="text-red-600">"{query}"</span>
                  </h2>
                  ...
                  <Link href="/" className="mt-10 px-8 py-4 bg-[#0A2647] text-white font-black rounded-2xl uppercase text-[11px] tracking-widest hover:bg-red-600 transition-all">
                    Volver al inicio
                  </Link>
                </div>
              </div>
              ...
              <>
                <nav className="mb-4 flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  ...
                  <span>/</span>
                  <Link href={`/buscar?query=${dynamicFilters.path.main}`} className="hover:text-[#0A2647] transition-colors">{dynamicFilters.path.main}</Link>
                  <span>/</span>
                  <Link href={`/buscar?query=${dynamicFilters.path.sub}`} className="hover:text-[#0A2647] transition-colors">{dynamicFilters.path.sub}</Link>
                ...
                <div className="mb-10 flex items-center justify-between gap-4">
                  <h1 className="text-2xl font-black text-[#0A2647] tracking-tighter capitalize">
                    Resultados <span className="text-slate-200"></span> <span className="text-red-600">{}</span>
                  </h1>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredItems.slice(0, visibleCount).map((item) => {
                    ...
                    const cardProduct = {
                      ...
                      slug: isInternal 
                          ? (item.slug || item.id)
                          : `${encodeURIComponent(item.id)}?src=ebay&title=${encodeURIComponent(item.title)}&price=${rawPrice}&image=${encodeURIComponent(imageUrl as string)}&url=${encodeURIComponent(item.url || "")}`,
                      title: item.title,
                    ...
                    return (
                      <ProductCard key={item.id} product={cardProduct} />
                    );
                  })}
                </div>
