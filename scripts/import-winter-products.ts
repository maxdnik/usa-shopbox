...
const SEED_PATH = path.join(process.cwd(), "seeds", "ski-snow.json");
...
function getWinterCategory(item: SeedItem): "ski" | "snowboard" | "outdoor" {
  ...
  const leaf = String(item.category?.leaf ?? "").toLowerCase().trim();
...
function getLengths(item: SeedItem): string[] {
  const lengths = [
    ...toStringArray(item.specs?.lengthsCm),
    ...toStringArray(item.specs?.sizeTable?.lengthCm),
  ]
    .map((v) => String(v).replace(/\.0$/, "").trim())
    .filter(Boolean);
...
function normalizeTags(item: SeedItem) {
  ...
  const winterType = getWinterCategory(item);
  tags.push(`winter:${winterType}`);

  if (item.brand) tags.push(`brand:${String(item.brand)}`);

  const terrain = toStringArray(item.specs?.terrain);
  for (const t of terrain) tags.push(`terrain:${t}`);

  if (item.specs?.skillLevel) tags.push(`skill:${String(item.specs.skillLevel)}`);
  if (item.specs?.profile) tags.push(`profile:${String(item.specs.profile)}`);
  if (item.specs?.shape) tags.push(`shape:${String(item.specs.shape)}`);
  if (item.specs?.flex) tags.push(`flex:${String(item.specs.flex)}`);
  ...
  for (const item of items) {
    try {
      ...
      const doc: any = {
        ...
        winterCategory,
        productType: item.type ?? winterCategory,
